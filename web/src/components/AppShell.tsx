  ...
  return (
    <div className="flex min-h-screen flex-col">
          ...
          <nav className="flex flex-1 flex-wrap items-center gap-1 text-sm">
            {links.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="rounded-lg px-3 py-2 text-white/85 hover:bg-white/10 hover:text-white"
              >
                {l.label}
              </Link>
            ))}
            {user.role === "admin" && (
              <Link href="/users" className="rounded-lg px-3 py-2 text-white/85 hover:bg-white/10">
                Users
              </Link>
            )}
          </nav>
      ...
      <footer className="flex flex-wrap items-center justify-center gap-2 border-t border-[color:var(--border)] px-3 py-3 text-sm text-[color:var(--muted)]">
        <Image src="/img/pfs-logo.png" alt="PFS Automation" width={90} height={28} className="h-7 w-auto rounded" />
        <span>·</span>
        <span>Powered by</span>
        <Image src="/img/casinworks-logo.png" alt="CasinWorks" width={100} height={22} className="h-5 w-auto opacity-80" />
      </footer>
    </div>
  );
}
